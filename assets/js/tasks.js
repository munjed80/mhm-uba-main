...
(function () {
  ...
  function renderTasksStandalone() {
    ...
    const matchStage = (needles, fallback) => {
      const found = columns.find((col) => needles.some((needle) => col.id.includes(needle)));
    ...
    if (inProgressBase && Array.isArray(inProgressBase.tasks)) inProgressTasks.push(...inProgressBase.tasks);
    if (reviewColumn && reviewColumn !== inProgressBase && Array.isArray(reviewColumn.tasks)) {
      inProgressTasks.push(...reviewColumn.tasks);
    }
    const inProgress = { id: 'inprogress', tasks: inProgressTasks };
    const doneColumn = matchStage(['done', 'complete', 'completed'], columns[columns.length - 1]);

    renderTaskList(backlogEl, backlogColumn, todayData.ids, 'No tasks in backlog.');
    renderTaskList(todayEl, todayData.tasks, null, 'Nothing due today.');
    renderTaskList(progressEl, inProgress, todayData.ids, 'No active tasks right now.');
    renderTaskList(doneEl, doneColumn, todayData.ids, 'No tasks marked done yet.');
  }
